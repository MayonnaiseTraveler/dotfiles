...
{  
    ...
    foreach ($entries as $i) {
        $basefile = basename($i);
        $basefiledir = pathinfo(str_replace(' ', '-' ,str_replace('/','-',str_replace('./blog/','',$i))), PATHINFO_FILENAME);
        ...
        switch ($mode) {
            case 1:
                //Print BlogIndex
                echo "<ul><h6 class='t-left'>";
                echo basename(dirname($i, 1));
                echo "</ul><hr style='margin-bottom:0t'>";
                printf("<br><li> %s - <a href='./blog.html#%s'> %s </a> </li>", $date, $basefiledir, $name);
                break;
            case 2:
                //Print BlogNews (latest 3 blogs in the index, without topics)
                if ($n < 3) {
                    printf("<li> %s - <a href='./blog.html#%s'> %s </a> </li>", $date, $basefiledir, $name);
                }
                $n++;
                break;
            case 3:
                //Print BlogPosts (Latest 3 Blog posts in the index)
                printf(" <article id='%s' class='cardnew clearfix gridblogpost'", $basefiledir);
                include $i;
                echo "</article>";
                break;
