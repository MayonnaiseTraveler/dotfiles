<!DOCTYPE HTML>
<html lang="en-US">

<head>

    <link rel="stylesheet" href="../css/projects.css">
    <meta property="og:image" content="https://bnnuyblood.neocities.org/screenshot.png" />
    <?php include '../components/generic/head.php'; ?>
</head>

<body class="gridmain">

    <?php include '../components/generic/navBar.php' ?>
    <header class="center" style="grid-area:title">
        <img class="title-img" src="../media/general/projects.png" alt="projects" >
    </header>

    <main class=" cardnew " style="grid-area:center">
        <header
            <img id="construction" src="../media/gifs/construction_board.gif" alt="">
        </header>
        <section class="center">

            <div class="project">
                <a href="./comic.php" target="_self" rel="noopener noreferrer">
                    <img class="project-img" src="../media/projects/comic/page0light_s.jpg" alt="Comic"
                </a>
                <h2 style="margin-top:1vw; font-family: 'Guttural'";>Duality</h2>
            </div>
            <div class="project">
                <a href="https://mayotraveller.itch.io/nyon" target="_blank" rel="noopener noreferrer">
                    <img class="project-img" src="../media/projects/nyon_s.jpg" alt="Coming Soon">
                </a>
                <h5>Old little game i made</h5>
            </div>

        </section>
    </main>
    <?php include '../components/generic/footer.php' ?>

</body>


</html>
